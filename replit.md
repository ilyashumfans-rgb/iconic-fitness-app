...
### Agency portal (read-only, all branches)

A standalone, view-only role for an agency to monitor GX class bookings across **every** branch. Single shared credential (no per-user accounts).

- **Auth:** session-based, mirroring admin (express-session, `user_sessions`). Credentials are env secrets `AGENCY_USERNAME` / `AGENCY_PASSWORD` (global → same login in dev + prod), compared with `crypto.timingSafeEqual`. Session value `agencyUser` on `SessionData`; `requireAgency` guards reads. Login regenerates the session, logout destroys it.
- **Lib:** `artifacts/api-server/src/lib/agencyAuth.ts` (`verifyAgencyCredentials`, `requireAgency`).
- **API:** `artifacts/api-server/src/routes/agency.ts` — `POST /agency/login`, `POST /agency/logout`, `GET /agency/me`, `GET /agency/gx-bookings`. The role is **read-only**: the only data endpoint returns all `leadsTable` rows with `kind='class'` (no gym filter, no write paths). Mounted in `routes/index.ts`.
- **Frontend:** standalone routes outside every member/partner/admin shell — `App.tsx` `if (location.startsWith("/agency"))` block → `/agency/login` (`pages/agency/Login.tsx`) + `/agency` dashboard (`pages/agency/Dashboard.tsx`). Client: `lib/agencyApi.ts`. Dashboard shows totals + breakdown **by branch** + **by class category** (`className`) + **per-slot detail** (branch+date+time+class with each booker's contact info).
- **Secrets must be set for login to work** — they are already configured; on a fresh clone re-add `AGENCY_USERNAME`/`AGENCY_PASSWORD`.
