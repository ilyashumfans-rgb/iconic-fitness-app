...
- **Login screen:** `app/(auth)/sign-in.tsx` is a premium branded landing — glowing Iconic brand-lockup card (`iconic-logo.png` + "iconic FITNESS / THE FITNESS COMPANY"), a "TRAIN ICONIC" headline + spaced subtitle, then the real auth (email/password → Clerk `signIn.password`, "Continue with Google" SSO, "Create account" link, "Continue without login" guest). No phone/Apple auth exists — the layout is inspired by a reference but only wires real methods; PRIVACY/TERMS footer opens `websiteUrl`.
