...
- **Plan curation (admin):** "YoActiv Plans" (`/admin/yoactiv-plans`, Membership Management nav) — branch picker → live plan catalog (membership + PT groups) with a visibility switch per plan. YoActiv owns names/prices; we only store a hidden flag in `yoactiv_package_prefs` (unique branch+package, `lib/yoactivPackagePrefs.ts`). Hidden plans are filtered from member-facing `GET /membership-packages` + `GET /trainer-packages` AND rejected in the purchase POST re-verify (can't buy hidden even with a crafted request). Admin routes `GET /admin/yoactiv/packages` + `PUT /admin/yoactiv/packages/:id/visibility` (requireAdmin, branch validated against configured sets).
