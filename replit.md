...
- **Bottom tabs:** Home, More (`app/(tabs)/_layout.tsx`). `train`/`classes`/`progress`/`profile` are still `(tabs)` routes but hidden from the bar via `options={{ href: null }}` and surfaced through the More hub (`app/(tabs)/more.tsx`). The hub lists Train, Classes, Progress, Profile (internal `router.push`) plus Sports & Fitness (→ `exploreUrl`) and Store (→ `storeUrl`), the latter two via `openExternal` (in-app browser).
