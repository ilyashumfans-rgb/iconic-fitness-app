...
- **Trial session request:** lime "Book your trial session" CTA atop mobile `app/trainers.tsx` roster → `book-trainer.tsx` `trial=1` mode (trial copy, lead `className="Trial session"`, same enquiry pipeline). `PtProgram.trainerPhotoUrl` (optional) = assigned trainer's staff-uploaded photo; Home journey card's "Trainer assignment" step becomes "Accepted by {name}" + round avatar once assigned.
