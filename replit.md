...
- **Login screen:** `app/(auth)/sign-in.tsx` is a cinematic full-bleed landing — a bespoke generated hero photo (`assets/images/auth-hero.png`, moody dark gym athlete with lime rim light) fills the screen behind a multi-stop `LinearGradient` scrim that fades to the solid dark background at the bottom. Brand mark + "ICONIC FITNESS" wordmark sit at top; a `flex` spacer lets the photo breathe; then an eyebrow ("WELCOME BACK") + bold headline and the real auth controls anchor to the bottom (email/password → Clerk `signIn.password`, gradient "Log in", "Continue with Google" SSO, "Create account" link, "Continue without login" guest). PRIVACY/TERMS footer opens `websiteUrl`. Uses `ImageBackground` + `KeyboardAvoidingView`/`ScrollView` (bounce disabled); `AppText` weight caps at "700".
