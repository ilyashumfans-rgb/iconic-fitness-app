...
- **Launch splash:** `components/AnimatedSplash.tsx` (brand mark + halo animation, hardcoded dark palette, fail-safe timeouts; enlarged 236px logo).
- **Staff credentials:** staff rows have optional unique `username` (set in admin Staff Management). `/staff/login` accepts `identifier` (email or username). `/staff/google-login` (mobile Bearer / web cookie via Clerk) admits only verified-primary-Gmail matching an existing active staff row — no auto-provisioning. Mobile studio login is `app/staff-login.tsx` at router root (Google flow needs a temporary Clerk session; `(auth)` layout would redirect).
- **Entry gate & studio login:** signed-out users land on `app/(auth)/welcome.tsx` (Membership Login → sign-in, Studio Login → `staff-login.tsx`, guest skip). Studio login reuses web staff accounts: `lib/staffSession.ts` posts `POST /api/staff/login` (cookie session, `credentials:"include"`, profile cached in AsyncStorage) → `app/staff-home.tsx` (verifies `GET /staff/me`, shows access/permissions, staff-portal WebView link, logout). `(tabs)/_layout` redirect target is `/(auth)/welcome`.
