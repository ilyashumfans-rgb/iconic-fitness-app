...
- **Login screen:** `app/(auth)/sign-in.tsx` is a premium branded landing — a glowing rounded card showing the full Iconic brand lockup image (`assets/images/iconic-lockup.png`, black-bg PNG so it blends into the black card) with a lime ambient glow behind it, then the real auth (email/password → Clerk `signIn.password`, "Continue with Google" SSO, "Create account" link, "Continue without login" guest). No headline text/phone/Apple auth — the layout is inspired by a reference but only wires real methods; PRIVACY/TERMS footer opens `websiteUrl`.
