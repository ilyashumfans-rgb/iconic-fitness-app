...
- **Staff-uploaded photos for YoActiv trainers & members:** YoActiv has no photos, so `trainer_photos`/`member_photos` tables key an uploaded `imageUrl` by YoActiv id (text, no FK). Helpers in `lib/trainerPhotos.ts`; `photoUrl` merged into admin/partner trainer+member listings and public `/trainers/live`. Mutations: `PUT|DELETE /admin|/partner/yoactiv/{trainers|members}/:id/photo` — partner routes take `branchId` and enforce owned branch + id present in that branch's live roster/member list; staff need the `bookings` permission (covered by the fail-closed `/partner/yoactiv` prefix guard). Web: Photo column with upload/remove in `YoactivMembersBrowser` (both views); mobile trainer list renders `photoUrl` with initials fallback.
