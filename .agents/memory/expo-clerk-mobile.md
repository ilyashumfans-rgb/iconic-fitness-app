...
OTP login (added Jul 2026): first-factor email OTP exists on SignInFuture — `signIn.emailCode.sendCode({emailAddress})` then `.verifyCode({code})` then check `status==='complete'` + `finalize()`. SMS/phone OTP is NOT supported by Replit-managed Clerk tenants — email code is the only OTP option. Method names aren't in the skill reference; confirm against `@clerk/shared` `dist/types/signInFuture.d.mts`.
