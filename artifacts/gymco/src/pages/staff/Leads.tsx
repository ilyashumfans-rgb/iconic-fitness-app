...
function StaffLeadsInner() {
  ...
  const update = async (id: number, patch: Record<string, unknown>) => {
    try {
      const updated = await staffApi.leads.update(id, patch);
  ...
  const remove = async (id: number) => {
    ...
    try {
      await staffApi.leads.remove(id);
...
export default function StaffLeads() {
  return (
    <PermissionGate perm="lead.manage">
      <StaffLeadsInner />
    </PermissionGate>
  );
}
