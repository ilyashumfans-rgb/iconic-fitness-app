...
export default function StaffTickets() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([staffApi.tickets.assigned(), staffApi.tickets.mine()])
      .then(([a, r]) => {
        setAssigned(a);
        setRaised(r);
      })
  ...
  const refreshDetail = async () => {
    ...
    setDetail(await staffApi.tickets.get(openId));
  };

  const create = async (input: NewTicketInput) => {
    setSubmitting(true);
    try {
      await staffApi.tickets.create(input);
      setCreating(false);
      setTab("raised");
      load();
    } catch (e) {
  ...
  const comment = async (text: string) => {
    ...
    setCommenting(true);
    try {
      await staffApi.tickets.comment(openId, text);
      await refreshDetail();
    } catch (e) {
      setErr((e as Error)?.message ?? String(e));
    } finally {
  ...
  const changeStatus = async (status: string) => {
    ...
    setStatusBusy(true);
    try {
      await staffApi.tickets.setStatus(openId, status);
      await refreshDetail();
      load();
    } catch (e) {
      setErr((e as Error)?.message ?? String(e));
  ...
  const isAssignee = detail?.ticket.assigneeRole === "staff";
