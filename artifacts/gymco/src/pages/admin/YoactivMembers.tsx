...
import {
  adminApi,
  type YoactivBranchOption,
  type YoactivMemberDetail,
  type YoactivMemberRow,
} from "@/lib/adminApi";
...
function MemberDetailRow({ mobile }: { mobile: string }) {
      ...
      <AdminCard className="p-6">
        ...
        {loadingBranches ? (
          ...
          <p className="text-sm text-slate-400">
            No YoActiv branches configured. Map gyms to their YoActiv Branch ID
            in Gym Management first.
          </p>
        ) : err ? (
    ...
    <>
      ...
      {expanded && (
        <tr className="border-t border-slate-800 bg-slate-900/60">
          <td colSpan={5}>
            {member.mobile.trim() ? (
              <MemberDetailRow mobile={member.mobile} />
            ) : (
              <p className="px-4 py-3 text-sm text-slate-400">
